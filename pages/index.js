...
import { Product, HeroBanner, CategoryNav, Footer } from "../components"
...
const nextPage = () => {
  setCurrentSliceStart(currentSliceStart + productsPerPage)
  setCurrentSliceEnd(currentSliceEnd + productsPerPage)
  setCurrentPage(currentPage + 1)
}
...
const filterData = (category) => {
  ...
  setCurrentSliceEnd(productsPerPage); // changed to 12
  setCurrentPage(1)
}
  ...
  return (
    <>
      ...
      <div className='products-heading'>
        <CategoryNav />
        <h1 className='text-gray-800 shadow-sm text-3xl pb-8 font-semibold'>{lang == 'en' ? 'Products ' : 'محصولات '}</h1>
        ...
        text-lg
        font-medium
        transition'>{lang == 'fa' ? 'همه' : 'All'}</div>
                ...
                {showlist &&
                  <div className="z-50 absolute top-14 -right-4  bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-500">
                    <ul className="py-2 text-sm text-gray-700 dark:text-gray-200">
                      ...
                      (<li key={i}>
                        <p onClick={() => { filterData(item?.category) }} className="block cursor-pointer px-4 py-2 hover:bg-gray-100 hover:text-gray-800">{lang == 'en' ? item.category : item.persian_cat}</p>
                      </li>
      ...
      <div className='my-16 w-full flex flex-row '>
        {currentSliceStart >= 4 && <button className='mx-auto  px-4 py-2 text-white rounded-lg bg-gradient-to-br from-black to-gray-800' onClick={previousPage}>{lang == 'en' ? 'Previous' : 'قبلی'}</button>}
        {currentSliceEnd < (filters ? products?.filter((item) => (item.category.category == filters)).length : products?.length) && <button className='mx-auto  px-4 py-2 rounded-lg text-white  bg-gradient-to-br from-black to-gray-800' onClick={nextPage}>{lang == 'en' ? 'Next' : 'بعد'}</button>}
      </div>
